...
const SEED_CANDIDATES = [
  {
    ...
    criminalCases: 0,
    manifesto: ['Infrastructure development', 'Women safety', 'Digital literacy'],
    imageInitial: 'P',
    age: 42,
  },
  {
...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ success: true, candidates: SEED_CANDIDATES, total: SEED_CANDIDATES.length });
  }
});
...
router.post('/compare', sanitizeBody, validators.candidateCompare, async (req, res) => {
  ...
  try {
    ...
    if (selected.length < 2) {
      return res.status(400).json({ success: false, error: 'Select at least 2 candidates to compare.' });
    }
    ...
    res.status(500).json({ success: false, error: 'Comparison failed. Please try again.' });
  }
});
