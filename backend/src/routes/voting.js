...
const { randomInt } = require("node:crypto"); // CSPRNG – safe for unpredictable identifiers
...
router.post(
  ...
  (req, res) => {
    ...
    const receipt = {
      ...
      message:
        "Your vote has been recorded in the EVM. A VVPAT slip has been generated for your verification.",
      disclaimer:
        "THIS IS A SIMULATION ONLY. No real votes are being recorded.",
    };
...
router.get("/receipt/:sessionId", (req, res) => {
  ...
  if (!receipt) {
    return res
      .status(404)
      .json({ success: false, error: "No vote found for this session." });
  }
  res.json({ success: true, receipt });
});
