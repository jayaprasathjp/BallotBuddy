...
const MOCK_CANDIDATES = [
  { id: 'cand-001', name: 'Priya Sharma', party: 'Progressive Alliance', partyColor: '#1976d2', symbol: '🌟' },
  { id: 'cand-002', name: 'Rajesh Kumar', party: 'National Unity Party', partyColor: '#f57c00', symbol: '🌅' },
  { id: 'cand-003', name: 'Meera Patel', party: 'Green Future Party', partyColor: '#388e3c', symbol: '🌿' },
  { id: 'cand-004', name: 'Arjun Singh', party: 'Peoples Voice Party', partyColor: '#7b1fa2', symbol: '🕊️' },
  { id: 'nota', name: 'NOTA', party: 'None of the Above', partyColor: '#94a3b8', symbol: '✖️' },
];
      ...
      <div className="container">
        ...
        <div className="vote-steps" role="list" aria-label="Voting steps">
          {['Select', 'Confirm', 'VVPAT', 'Receipt'].map((s, i) => {
            const stepMap = { Select: 'select', Confirm: 'confirm', VVPAT: 'vvpat', Receipt: 'receipt' };
            ...
            return (
              <div key={s} className={`vstep ${isDone ? 'done' : ''} ${isCurrent ? 'current' : ''}`} role="listitem" aria-current={isCurrent ? 'step' : undefined}>
        ...
        {step === 'select' && (
          <div className="ballot-paper" role="main" aria-label="Ballot paper">
            ...
            {error && <div className="vote-error" role="alert">{error}</div>}

            <div className="ballot-candidates" role="radiogroup" aria-label="Candidate selection">
              {MOCK_CANDIDATES.map((c) => (
                ...
                >
                  <div className="ballot-symbol" style={{ background: `${c.partyColor}20`, color: c.partyColor }} aria-hidden="true">
                    {c.symbol}
                  </div>
                  <div className="ballot-name">
                    <div className="ballot-cand-name">{c.name}</div>
                    <div className="ballot-party" style={{ color: c.partyColor }}>{c.party}</div>
                  </div>
                  <button
            ...
            <button
              ...
              aria-label={selectedCandidate ? `Proceed to confirm vote for ${selectedCandidate.name}` : 'Select a candidate first'}
            >
              {t('vote.cast_vote')} →
            </button>
          </div>
        ...
        {step === 'confirm' && selectedCandidate && (
          <div className="confirm-panel" role="dialog" aria-labelledby="confirm-heading" aria-modal="true">
            <h2 id="confirm-heading" className="confirm-title">⚠️ {t('vote.confirm_vote')}</h2>
            <div className="confirm-card" style={{ borderColor: selectedCandidate.partyColor }}>
              <div className="confirm-symbol" style={{ background: `${selectedCandidate.partyColor}20`, color: selectedCandidate.partyColor }}>
                {selectedCandidate.symbol}
              </div>
              <div className="confirm-info">
                <div className="confirm-name">{selectedCandidate.name}</div>
                <div className="confirm-party" style={{ color: selectedCandidate.partyColor }}>{selectedCandidate.party}</div>
              </div>
            </div>
            <p className="confirm-warning">Once submitted, your vote cannot be changed. Are you sure?</p>
            <div className="confirm-actions">
              <button className="btn btn-secondary" onClick={() => setStep('select')} aria-label="Go back and change selection">
                ← Change
              </button>
              <button
        ...
        {/* VVPAT Animation */}
        {step === 'vvpat' && (
          <div className="vvpat-panel" role="status" aria-live="polite" aria-label="VVPAT paper slip being printed">
            ...
            <p className="vvpat-note">Your vote has been recorded. Verify the printed slip.</p>
          </div>
        )}
        ...
        {step === 'receipt' && receipt && (
          <div className="receipt-panel" role="main" aria-label="Vote receipt">
            ...
            <div className="receipt-card" role="article" aria-label="VVPAT receipt details">
              ...
              <div className="receipt-row">
                ...
                <code>{new Date(receipt.timestamp).toLocaleString('en-IN')}</code>
              </div>
              <div className="receipt-disclaimer" role="note">{receipt.disclaimer}</div>
            </div>

            <button className="btn btn-secondary" onClick={resetVote} aria-label="Start over and vote again">
              🔄 {t('vote.back')}
            </button>
          </div>
        )}
