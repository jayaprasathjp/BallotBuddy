...
const SUGGESTED_QUESTIONS = [
  'chat.suggested_q1', 'chat.suggested_q2', 'chat.suggested_q3', 'chat.suggested_q4'
];
  ...
  return (
    <div className="ai-response-card" role="article" aria-label="AI response">
      <div className="ai-response-header">
        ...
        <button className="speak-btn" onClick={() => speak(fullText)} aria-label={t('chat.speak')} title={t('chat.speak')}>
          🔊
        </button>
      </div>
        ...
        <div className="response-section">
          ...
          <ol className="response-steps" aria-label="Step-by-step guide">
            {response.steps.map((step, i) => (
              <li key={i} className="response-step">{step.replace(/^Step \d+:\s*/i, '')}</li>
            ))}
          </ol>
        </div>
      )}
      ...
      {response.relatedTopics?.length > 0 && (
        <div className="related-topics">
          ...
          {response.relatedTopics.map((topic) => (
            <span key={topic} className="related-chip">{topic}</span>
          ))}
        </div>
      )}
    </div>
...
export default function ChatPage() {
  ...
  const sendMessage = useCallback(async (messageText) => {
    ...
    try {
      ...
      const aiMsg = { role: 'model', response: res.data.response, id: Date.now() + 1 };
          ...
          <div className="chat-messages" role="log" aria-live="polite" aria-label="Chat messages">
            {messages.length === 0 && (
              <div className="chat-empty" role="status">
                ...
                <p>I can help with voter registration, election steps, documents needed, and more.</p>
              </div>
            )}
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`message-row ${msg.role}`}>
                {msg.role === 'user' ? (
                  <div className="user-bubble" role="article" aria-label={`You said: ${msg.content}`}>
                    {msg.content}
                  </div>
                ) : (
            ...
            {isLoading && (
              <div className="message-row model" role="status" aria-label={t('chat.thinking')}>
            ...
            <div className="chat-controls">
              ...
              <div className="chat-buttons">
                <button
                  ...
                  aria-label={isListening ? t('chat.voice_stop') : t('chat.voice_start')}
