...
const TestComponent = () => {
  const { highContrast, setHighContrast, fontScale, increaseFontSize, decreaseFontSize, resetFontSize } = useAccessibility();
  return (
    <div>
      <div data-testid="contrast">{highContrast ? 'high' : 'normal'}</div>
      <div data-testid="scale">{Math.round(fontScale * 100)}</div>
      <button onClick={() => setHighContrast((v) => !v)}>Toggle Contrast</button>
